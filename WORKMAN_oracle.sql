...
select no, id, to_char(regdate, 'YYYY-DD-MM HHmmss') from member order by regdate desc;

SELECT LISTAGG(NAME,', ') WITHIN GROUP(ORDER BY NAME) AS HASHTAGS
  FROM HASHTAG JOIN HASHTAG_JOB
  ON HASHTAGNO = NO AND BOARDNO=148;
